...
int MWBotWin::checkCooldown(QWebElement& elm, QString name) {
...
void MWBotWin::rideCar() {
	...
	curTime = QDateTime::currentDateTime();
	if (num == 0) {
		log(trUtf8("Нет машин в гараже. Поездки отключены."));
		opt.car.ride = 0;
	} else {
		elm = frm->findFirstElement("div.home-garage dd div.about button.button span.f div.c");
		clickElement(elm);		// click 'go ride' button
		...
		foreach(rid, elm.findAll("li[id^=direction]")) {						// for each ride in list
			if (rid.findFirst("i.icon-locked").isNull()) {						// check ride isn't locked
				...
				if (checkName(name, opt.car.list)) {
					if (checkCooldown(rid, name) == 0) {
						do {
							rpt = 0;
							...
							if (line.contains(trUtf8("пустым баком"),Qt::CaseInsensitive)) {
								...
								res.result = resBonus;
								logResult(res);
								clickElement("div.alert.infoalert div.data div.actions div.button span.f div.c");		// close alert window
								// ui.browser->reload();
								checkCooldown(rid, name);
							} else {
								qDebug() << "some alert";
							}
						} while (rpt);
